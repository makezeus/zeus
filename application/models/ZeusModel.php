...
class ZeusModel extends CI_Model
{
    ...
    function getBilledProducts()
    {

        $sql = "SELECT produtos.idProdutos, 
        produtos.descricao, 
        SUM(produtos_os.quantidade) AS total
        FROM produtos
        INNER JOIN produtos_os ON produtos.idProdutos = produtos_os.produtos_id
        INNER JOIN os ON produtos_os.os_id = os.idOs
        WHERE os.status = 'Faturado'
        GROUP BY produtos.idProdutos, produtos.descricao ORDER BY total desc LIMIT 5;";
    ...
    function getProdutos()
    {
        ...
        $sql = "SELECT * FROM produtos order by idProdutos desc LIMIT 5";
        return $this->db->query($sql)->result();
    }
